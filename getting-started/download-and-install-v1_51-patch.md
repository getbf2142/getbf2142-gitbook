# ② Install v1.51 Patch

In this tutorial, we’ll walk you through the steps to update the game to the latest version.

{% hint style="warning" %}
You can skip this tutorial if your game is already patched to v1.51. If you still need the disc to launch the game, that means you haven’t updated to v1.51 yet.
{% endhint %}

{% hint style="info" %}
If you’re ever unsure about your game’s version, just check the version string in this file:\
`C:\Program Files (x86)\Electronic Arts\Battlefield 2142\mods\bf2142\Mod.desc`
{% endhint %}

<details>

<summary>Why is it necessary to install v1.51 patch?</summary>

* [**DRM**](#user-content-fn-1)[^1] **is officially removed (No-CD Patch):** You no longer need the game disc to launch the game. For more info, see the [No-CD Patch](https://tweakguides.dmegaming.com/BF2142_4.html) section in Tweak Guide.

- **Widescreen support:** Enjoy a more immersive field of view and easier multitasking.

* **Northern Strike expansion included for free:** A final goodbye to the game as DICE moved on to Battlefield 3 and beyond.

- **Access to all final features:** This patch unlocks the game’s last features, bug fixes, and extra content.

* **Compatibility:** Most current master servers (like OpenSpy), game servers (such as Reclamation), and popular mods (like Project Remaster) require v1.51.

</details>

## Procedures

1. Download <mark style="color:blue;">BF2142\_Update\_1.51\_Full.exe</mark> from [Downloads](download-and-install-v1_51-patch.md#downloads) → Full Installer.
2. Double-click the file to start the installer - it may take a moment for anything to appear. \[Why?[^2]]
3. When prompted by [<mark style="color:blue;">User Account Contro</mark>](#user-content-fn-3)[^3]<mark style="color:blue;">l</mark>, click <mark style="color:blue;">Yes</mark> to allow the installer to make changes on your device.
4. If <mark style="color:blue;">Windows Defender SmartScreen</mark> shows <mark style="color:blue;">Windows Protected Your PC</mark>, click <mark style="color:blue;">More Info</mark> and then <mark style="color:blue;">Run Anyway</mark>. If you don't see the option, then \[...[^4]]
5. Follow the on-screen instructions in the wizard to finish the installation.​
6. Next, make sure to [install BF2142 Hub](download-and-install-bf2142-hub.md) and [apply OpenSpy Patches](apply-openspy-patches.md) — _these steps are crucial, so don't skip them_! \[Why?[^5]]

## Downloads

{% tabs %}
{% tab title="Full Installer" %}
This installer will update your game to the latest version (v1.51), no matter which version you currently have — whether it’s v1.00, v1.01, or v1.50.&#x20;

Unlike the official updater, which only works incrementally, this installer skips all the intermediate steps, making the update process much simpler and more user-friendly.

{% hint style="danger" %}
You might see virus warnings when launching this installer. If you’re concerned about file safety, you can use the incremental installer mirrored from EA instead.
{% endhint %}

{% embed url="https://www.moddb.com/games/battlefield-2142/downloads/battlefield-2142-update-v151-full" %}
**BF2142\_Update\_1.51\_Full.exe (ModDB, 2.3 GB) -** Source: [LordSte](https://www.moddb.com/members/lordste) on [ModDB](https://www.moddb.com/)
{% endembed %}
{% endtab %}

{% tab title="Incremental Installer" %}
The installers provided here are mirrors of the originals from EA, made available before the game’s termination. To update your game to the latest version, first install the v1.50 patch, then follow up with the v1.51 patch.

The v1.50 patch upgrades your game from v1.00 or v1.01 to v1.50, and the v1.51 patch takes it from v1.50 to v1.51.

{% hint style="danger" %}
Make sure to install v1.50 patch first, and then follow it with v1.51 patch to update your game in the correct order.
{% endhint %}

{% embed url="https://www.moddb.com/games/battlefield-2142/downloads/battlefield-2142-update-150" %}
**BF2142\_Update\_**<mark style="color:red;">**1.50**</mark>**.exe (ModDB, 523 MB) -** Source: [RhodN](https://www.moddb.com/members/grafix1) on [ModDB](https://www.moddb.com/)
{% endembed %}

{% embed url="https://www.moddb.com/games/battlefield-2142/downloads/battlefield-2142-v151-patch-final" %}
**BF2142\_Patch\_**<mark style="color:red;">**1.51**</mark>**.exe (ModDB, 1.95 GB) -** Source: [ytres](https://www.moddb.com/members/sdgsdgsdgsdg) on [ModDB](https://www.moddb.com/)
{% endembed %}
{% endtab %}
{% endtabs %}

[^1]: **DRM is an anti-piracy mechanism that requires players to launch their games with their game discs to verify their premium identity and play online.**

[^2]: The installer takes time to decompress files to prepare for the installation.

[^3]: i.e., <mark style="color:blue;">Do you want to allow this app from an unknown publisher to make changes to your device?</mark>

[^4]: If you don’t see the <mark style="color:blue;">More Info</mark> option, right-click the installer and select <mark style="color:blue;">Properties</mark>. Check the <mark style="color:blue;">Unlock</mark> box, then click <mark style="color:blue;">Apply</mark> and <mark style="color:blue;">OK</mark>. After that, try running the installer again starting from step 2.

[^5]: If you skip these steps, you won’t be able to log in, since EA’s online services for this game have been shut down.

...
* You’ll be editing files in `mods\<MOD>\AI\AIDefault.ai`, so it’s a good idea to make a backup of the file first — just add a suffix like `_bak` or `_o` to the filename of the clone. \[Why?[^1]]
...
1. Inside the `AI` folder, open `AIDefault.ai` with a text editor.
2. Look for the line `aiSettings.setMaxNBots 16` near the top of the file.
3. Change the number to however many bots you want — for example, `aiSettings.setMaxNBots 32` for 32 bots. Keep in mind that \[...[^2]]
4. Make sure to set `aiSettings.maxBotsIncludeHumans` to 0. \[Why?[^3]]
5. Put `aiSettings.overrideMenuSettings 1` before the line with `aiSettings.maxBotsIncludeHumans 0`. \[Why?[^4]]
6.  Your file should look something like this around lines 9–12:
    ...
    ```
7. You can also adjust the bot skill — set `aiSettings.setBotSkill` to any value between 0 and 1 (higher means tougher bots).
8. Once you’re done, just save your changes!
